...
/* Gets a double configuration value #### NOT WORKING #########.
 * This can be:
 *  - A Range widget's current value
 *
 * Sample (for Canons eg):
 *   get_config_value_double (camera, "Zoom", &zoomVal, context);
 */
int
get_config_value_double (Camera *camera, const char *key, double *dbl, GPContext *context) {
    CameraWidget		*widget = NULL, *child = NULL;
    CameraWidgetType	type;
    int			ret;
    ...
    /* This is the actual query call. Note that we just
     * a pointer reference to the string, not a copy... */
    ret = gp_widget_get_value (child, dbl);
    if (ret < GP_OK) {
        fprintf (stderr, "could not query widget value: %d\n", ret);
        goto out;
    }
